...
import { ESTADO_PERMISO } from '@/hooks/rrhh/useSolicitudes';
 ...
 * - En espera (1) usa lider=1 → muestra botones aprobar/rechazar.
...
/**
 * Diálogo "Permisos de un usuario".
 *
 * Muestra los permisos del empleado seleccionado con filtros por estado
 * (En espera / Aprobados / Rechazados / Finalizados). En el estado
 * "En espera" se muestran las acciones Aprobar / Rechazar.
 *
 * Props:
 *  - open          {boolean}
 *  - onClose       {function}
 *  - usuario       {object}  Empleado seleccionado: { idUsuario, nombre }.
 *  - rows          {Array}   Permisos del estado activo.
 *  - loading       {boolean}
 *  - estadoActivo  {number}  Estado actualmente seleccionado.
 *  - onCambiarEstado {function(estadoId, lider)}  Cambia el filtro.
 *  - onAprobar     {function(row)}  Aprueba un permiso.
 *  - onRechazar    {function(row)}  Rechaza un permiso.
 *  - accionandoId  {string|number|null}  Id del permiso en proceso de aprobar/rechazar.
 */
export default function PermisosUsuarioDialog({
    open,
    onClose,
    usuario = {},
    rows = [],
    loading = false,
    estadoActivo = ESTADO_PERMISO.EN_ESPERA,
    onCambiarEstado,
    onAprobar,
    onRechazar,
    accionandoId = null,
    ...
    const esEnEspera = estadoActivo === ESTADO_PERMISO.EN_ESPERA;
                ...
                >
                    {FILTROS.map((f) => (
                        <Button
                            ...
                            color={
                                estadoActivo === f.id
                                    ? f.id === ESTADO_PERMISO.APROBADO
                                        ? 'success'
                                        : f.id === ESTADO_PERMISO.RECHAZADO
                                            ? 'error'
                                            : f.id === ESTADO_PERMISO.FINALIZADO
                                                ? 'info'
                                                : 'primary'
                                    : 'inherit'
                            }
