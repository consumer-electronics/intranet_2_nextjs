...
/**
 * Tabla de personal (Personal a cargo / Todo el personal).
 *
 * Props:
 *  - rows        {Array}    Filas: { id, idUsuario, nombre, permisosPendientes, cartaRenuncia }.
 *  - loading     {boolean}  Estado de carga.
 *  - onRowClick  {function} Callback al hacer clic en una fila (abre permisos del usuario).
 *  - exportName  {string}   Nombre base del archivo Excel.
 *
 * Ref expuesto:
 *  - triggerExport() → descarga el Excel respetando filtros/orden actuales.
 */
const PersonalTable = forwardRef(function PersonalTable(
    { rows, loading, onRowClick, exportName = 'personal-permisos' },
    ref
) {
    ...
    const columns = [
        ...
        },
        {
            ...
            sortable: true,
            renderCell: (params) => {
                const tiene = String(params.value ?? '').toLowerCase().includes('sí');
                return (
                    <Chip
                        size="small"
                        label={tiene ? 'Sí' : 'No'}
